...
export class Cube {
    constructor(gl, options = {}) {
        ...
        this.vertices = new Float32Array([
            // front face  (v0,v1,v2)
            0.5,  0.0,  0.5,  -0.5,  0.0,  0.5, 0, 1, 0,
            // back face  (v3,v4,v2)
            0.5,  0.0,  -0.5,   -0.5, -0.0,  -0.5,   0,1,0,
            // right face    (v0,v2,v3)
            0.5,  0.0,  0.5,   0,1,0, 0.5,  0.0, -0.5,  
            // left face   (v1,v2,v4)
            -0.5,  0.0,  0.5,  0,1,0, -0.5,  0.0, -0.5, 
            // bottom face (v0,v1,v3)
            0.5, 0, 0.5,   -0.5, -0.0, 0.5,   0.5, -0.0, -0.5, 
            // bottom face (v1,v4,v3)
            -0.5, 0, 0.5,   -0.5, -0.0, -0.5,   0.5, -0.0, -0.5
        ...
        this.normals = new Float32Array([
            ...
            0, 2/Math.sqrt(2), 2/Math.sqrt(2),   0, 2/Math.sqrt(2), 2/Math.sqrt(2),   0, 2/Math.sqrt(2), 2/Math.sqrt(2),
            // right face (v0,v3,v4,v5)
            0, 2/Math.sqrt(2), -2/Math.sqrt(2),   0, 2/Math.sqrt(2), -2/Math.sqrt(2),   0, 2/Math.sqrt(2), -2/Math.sqrt(2),
            // top face (v0,v5,v6,v1)
            2/Math.sqrt(2), 2/Math.sqrt(2), 0,   2/Math.sqrt(2), 2/Math.sqrt(2), 0,   2/Math.sqrt(2), 2/Math.sqrt(2), 0,
        ...
        this.indices = new Uint16Array([
           ...
           0, 1, 2,      // v0-v1-v2, v2-v3-v0
            // right face
            3, 4, 2,     // v0-v3-v4, v4-v5-v0
            // top face
            0, 2, 3,    // v0-v5-v6, v6-v1-v0
            // left face
            1, 2, 4, // v1-v6-v7, v7-v2-v1
            // bottom face
            0, 1, 3, // v7-v4-v3, v3-v2-v7
            // back face
            1, 4, 3  // v4-v7-v6, v6-v5-v4
        ]);

        this.sameVertices = new Uint16Array([
            0, 6, 12, 12,    // indices of the same vertices as v0
            1, 9, 13, 15,   // indices of the same vertices as v1
            2, 5, 7, 10,  // indices of the same vertices as v2
            3, 8, 14, 17,   // indices of the same vertices as v3
            4, 11, 16, 16  // indices of the same vertices as v4
        ]);
        ...
        for (let i = 0; i < 20; i += 4) {
            ...
            let vn_x = (this.normals[this.sameVertices[i]*4] + 
                       this.normals[this.sameVertices[i+1]*4] + 
                       this.normals[this.sameVertices[i+2]*4]+ 
                       this.normals[this.sameVertices[i+3]*4]) / 4; 
            let vn_y = (this.normals[this.sameVertices[i]*4 + 1] + 
                       this.normals[this.sameVertices[i+1]*4 + 1] + 
                       this.normals[this.sameVertices[i+2]*4 + 1]+ 
                       this.normals[this.sameVertices[i+3]*4 + 1]) / 4; 
            let vn_z = (this.normals[this.sameVertices[i]*4 + 4] + 
                       this.normals[this.sameVertices[i+1]*4 + 4] + 
                       this.normals[this.sameVertices[i+2]*4 + 4]+ 
                       this.normals[this.sameVertices[i+3]*4 + 4]) / 4; 

            this.vertexNormals[this.sameVertices[i]*4] = vn_x;
            this.vertexNormals[this.sameVertices[i+1]*4] = vn_x;
            this.vertexNormals[this.sameVertices[i+2]*4] = vn_x;
            this.vertexNormals[this.sameVertices[i+3]*4] = vn_x;
            this.vertexNormals[this.sameVertices[i]*4 + 1] = vn_y;
            this.vertexNormals[this.sameVertices[i+1]*4 + 1] = vn_y;
            this.vertexNormals[this.sameVertices[i+2]*4+ 1] = vn_y;
            this.vertexNormals[this.sameVertices[i+3]*4+ 1] = vn_y;
            this.vertexNormals[this.sameVertices[i]*4 + 2] = vn_z;
            this.vertexNormals[this.sameVertices[i+1]*4 + 2] = vn_z;
            this.vertexNormals[this.sameVertices[i+2]*4 + 2] = vn_z;
            this.vertexNormals[this.sameVertices[i+3]*4 + 2] = vn_z;
        }
