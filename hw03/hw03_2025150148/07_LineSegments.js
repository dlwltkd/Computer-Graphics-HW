...
function setupMouseEvents() {
    ...
    function handleMouseUp() {
        if (isDrawing && tempEndPoint) {
            ...
            let distance = Math.sqrt((tempEndPoint[0] - startPoint[0])**2 + (tempEndPoint[1] - startPoint[1])**2);
            for (let i = 0; i<= 1000; i++){
                lines.push([])
                lines[drawed].push(startPoint[0] + distance * Math.cos(Math.PI/500 * i), startPoint[1] + distance * Math.sin(Math.PI/500 * i));
            }
            drawed++;
...
function render() {
    ...
    gl.bindVertexArray(vao);
    gl.drawArrays(gl.LINE_STRIP, 1001*(drawed-1), 1001);}

    // 임시 선 그리기
    if (isDrawing && startPoint && tempEndPoint) {
        shader.setVec4("u_color", [0.5, 0.5, 0.5, 1.0]); // 임시 선분의 color는 회색
        let distances = Math.sqrt((tempEndPoint[0] - startPoint[0])**2 + (tempEndPoint[1] - startPoint[1])**2);
        let templines = [];
            for (let i = 0; i<= 1000; i++){
                templines.push(startPoint[0] + distances * Math.cos(Math.PI/500 * i), startPoint[1] + distances * Math.sin(Math.PI/500 * i));
            }
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(templines), 
                      gl.STATIC_DRAW);
        gl.bindVertexArray(vao);
